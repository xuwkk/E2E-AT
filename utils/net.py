...
#     # it will be done in NN_SPO
#     def __init__(self, prob, b_default):
#         """
...
#                 )
#         self.b_default = torch.from_numpy(b_default).float()
        
#     def forward(self, load_forecast, b = None):
#         # TODO: not sure how the parameter is listed in order?
        
#         if b is None:
#             b = self.b_default.repeat(load_forecast.shape[0], 1)
#         return self.layer(b, load_forecast)[0]
...
#         # load_shedding, generator_storage
#         # TODO: not sure how the parameter is listed in order?
#         outputs = self.layer(b, pg, load_true)
#         return outputs[0], outputs[1]

# class SPO(nn.Module):
#     """
#     the convex differentiable layer with two-stage optimization
#     given the parameter b (susceptance), the layer will optimize the load shedding and generator storage
#     """
#     def __init__(self, operator, fix_first_b):
#         self.fix_first_b = fix_first_b
#         self.stage_one_layer = Stage_One_Layer(operator.prob_1, operator.b)
#         self.stage_two_layer = Stage_Two_Layer(operator.prob_2)
    
#     def forward(self, load, b):
#         # load: forecast or true load which should NOT be normalized
#         if self.fix_first_b:
#             # b not pass to the fisrt stage opt
#             pg = self.stage_one_layer(load)
#         else:
#             pg = self.stage_one_layer(load, b)
        
#         # stage two: pass b
#         ls, gs = self.stage_two_layer(pg, load, b)
