...
# class PGD_INPUT:
#     """
#     attack on the input of the load forecaster
#     the input has been normalized into 0 to 1
#     """
#     def __init__(self, operator, is_spo, max_eps, no_iter, fix_first_b, flexible_feature, fixed_feature):
...
#         self.flexible_feature = flexible_feature # index of the feature that can be attacked
#         self.fixed_feature = fixed_feature
        
#         self.step_size = 1 / no_iter
#         self.is_spo = is_spo
...
#         X_min[:, :, :, self.flexible_feature] = (X_min[:, :, :, self.flexible_feature] - self.max_eps).clamp(0, 1)
#         X_max[:, :, :,  self.flexible_feature] = (X_max[:, :, :, self.flexible_feature] + self.max_eps).clamp(0, 1)
...
#     def attack(self, nn, feature, target):
#         """
#         nn: neural network with spo
#         feature: normalized input
#         target: ground truth load (scaled)
#         """
#         nn.eval()
        
#         X_min, X_max = self.initial_bound_clamp(feature)
#         X_min_flex, X_max_flex = X_min[:, :, :, self.flexible_feature], X_max[:, :, :, self.flexible_feature]
        
#         X_flex = torch.rand_like(X_min_flex) * (X_max_flex - X_min_flex) + X_min_flex # initialize the input
#         X_flex.requires_grad = True
        
#         # X_flex = self.initialize_attack(feature)
        
#         X_fix = feature[:, :, :, self.fixed_feature]
#         X_fix.requires_grad = False
        
#         for t in range(self.no_iter):
#             with torch.enable_grad():
#                 X_att = torch.cat([X_fix, X_flex], dim = -1)
#                 if self.is_spo:
#                     forecast_load, pg, ls, gs = nn(X_att, target, self.b_default.repeat(target.shape[0], 1))
#                     loss = self.loss_spo(pg, ls, gs)
#                 else:
#                     forecast_load = nn(X_att)
#                     loss = self.loss_mse(forecast_load, target)
            
#             loss.backward()
#             # gradient ascent
#             X_flex.data = X_flex.data + self.step_size * X_flex.grad.data.sign()
#             # projection
#             X_flex.data.clamp_(min = X_min_flex, max = X_max_flex)

#             X_flex.grad.zero_()
        
#         X_att = torch.cat([X_fix, X_flex], dim = -1).detach()
...
#         if self.is_spo:
#             # attack targetting on the cost
#             forecast_load, pg, ls, gs = nn(X_att, target, self.b_default.repeat(target.shape[0], 1))
#             loss = self.loss_spo(pg, ls, gs)
#         else:
#             # attack targetting on the mse
#             forecast_load = nn(X_att)
#             loss = self.loss_mse(forecast_load, target)
...
# class PGD:
#     """
#     given the decision, attack on the susceptance of the stage-two problem
#     """
#     def __init__(self, operator, max_ratio, no_iter, fix_first_b):
#         """
#         nn: the neural network of stage two problem
#         fix_first_b: whether to fix the b in the stage one problem
#         """

#         b_default = torch.from_numpy(operator.b).float() 
#         self.b_min = b_default * (1 - max_ratio)
#         self.b_max = b_default * (1 + max_ratio)
#         self.b_default = b_default
#         self.no_iter = no_iter
#         self.fix_first_b = fix_first_b
        
#         # adaptively choose the step size for different b
#         self.step_size = (self.b_max - self.b_min) / no_iter
...
#         # try to maximize the load shedding and generation storage loss
#         if self.fix_first_b:
#             loss = ls.sum(axis=1) * self.load_shed_coeff + gs.sum(axis=1) * self.gen_storage_coeff
#         else:   
#             loss = pg @ self.first_coeff + ls.sum(axis=1) * self.load_shed_coeff + gs.sum(axis=1) * self.gen_storage_coeff
#         return loss
    
#     def attack(self, nn, input, target):
#         """
#         input can be either feature or pg
#         """
#         # nn = deepcopy(nn).eval()
#         nn.eval()
#         b_att = deepcopy(self.b_default).repeat(target.shape[0], 1) # start from the default susceptance
        
#         b_att.requires_grad = True
#         for _ in range(self.no_iter):
#             with torch.enable_grad():
#                 if self.fix_first_b:
#                     # only attack on the second stage problem
#                     ls, gs = nn(input, target, b_att)
#                     loss = torch.mean(self.loss(None, ls, gs))
#                 else:
#                     # attack on the both problems
#                     forecast_load, pg, ls, gs = nn(input, target, b_att)
#                     loss = torch.mean(self.loss(pg, ls, gs))
            
#             loss.backward()
#             # each susceptance is updated according to their attack range
#             att_vector = self.step_size.repeat(target.shape[0], 1) * b_att.grad.data.sign()
#             b_att.data = b_att.data + att_vector # size: (batch_size, no_branch) gradient ascent
...
#     def __init__(self, first_coeff, load_shed_coeff, gen_storage_coeff, b_default, max_ratio, no_iter):
#         """
#         nn: the neural network of spo type
#         """
#         b_default = torch.from_numpy(b_default).float() if type(b_default) == np.ndarray else b_default
#         self.b_min = b_default * (1 - max_ratio)
#         self.b_max = b_default * (1 + max_ratio)
#         self.no_iter = no_iter
...
#         self.first_coeff = torch.tensor(first_coeff, dtype=torch.float)
#         self.load_shed_coeff = torch.tensor(load_shed_coeff, dtype = torch.float)
#         self.gen_storage_coeff = torch.tensor(gen_storage_coeff, dtype = torch.float)
        
#         # self.relative_scale = b_default / b_default.max()
...
#     def find(self, nn, feature, target):
#         # nn = deepcopy(nn).eval()
#         nn.eval()
#         b_att = deepcopy(self.b_min).repeat(feature.shape[0], 1)
#         b_att.requires_grad = True
#         for i in range(self.no_iter):
#             with torch.enable_grad():
#                 _, pg, ls, gs = nn(feature, target, b_att)
#                 loss = torch.mean(self.loss(pg, ls, gs))
            
#             loss.backward()
#             # each susceptance is updated by the same ratio
#             # print(self.step_size.repeat(feature.shape[0], 1))
#             # print(b_att.grad.data.sign())
#             att_vector = self.step_size.repeat(feature.shape[0], 1) * b_att.grad.data.sign()
#             b_att.data = b_att.data + att_vector # size: (batch_size, no_branch)
